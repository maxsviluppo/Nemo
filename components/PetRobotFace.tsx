...
const PetRobotFace: React.FC<PetRobotFaceProps> = ({
  ...
  const soundJitter = useMemo(() => {
    ...
    const activeMultiplier = (lookDirection !== 'center' || targetObject || mousePos) ? 0.15 : 1;
    return {
  ...
  }, [volume, lookDirection, targetObject, mousePos]);

  // Simulazione dello sguardo biologico: Saccadi, Drift e Fissazioni
  useEffect(() => {
    const updateGaze = () => {
      // Prioritize explicit Emotions that override gaze
      if (
      ...
      if (currentEmotion === Emotion.ANXIOUS) {
        ...
        // --- MOUSE TRACKING (Highest Priority for Attention) ---
        // Map normalized -1..1 coordinates to eye movement range
        const rangeX = 140; // Max horizontal look
        const rangeY = 70;  // Max vertical look
        targetX = mousePos.x * rangeX;
        targetY = mousePos.y * rangeY;
        targetRotate = targetX * 0.08; // Head tilt effect
        targetScanScale = 1.15; // Slightly focused pupils
        nextJump = 100; // Fast update rate for smooth tracking
    ...
    const applyMicroSaccade = () => {
      const stabilityFactor = !!targetObject || !!mousePos || isDrifting ? 0.2 : (currentEmotion === Emotion.ANXIOUS ? 2.5 : 1);
      setLookOffset(prev => ({
        ...prev,
  ...
  }, [currentEmotion, lookDirection, targetObject, volume, isDrifting, mousePos]);
  ...
  const renderEyeShape = (emotion: Emotion, side: 'left' | 'right') => {
    switch (emotion) {
      ...
      case Emotion.CAT: return <EyeParts.CatEye />;
      case Emotion.DOG: return <EyeParts.DogEye />;
      case Emotion.ELEPHANT: return <EyeParts.ElephantEye />;
      case Emotion.ANT: return <EyeParts.AntEye />;
      case Emotion.MONKEY: return <EyeParts.MonkeyEye />;
      case Emotion.BIRD: return <EyeParts.BirdEye />;
      // New 10
      case Emotion.ALIEN: return <EyeParts.AlienEye />;
      case Emotion.GHOST: return <EyeParts.GhostEye />;
      case Emotion.FIRE: return <EyeParts.FireEye />;
      case Emotion.MUSIC: return <EyeParts.MusicEye />;
      case Emotion.BROKEN: return <EyeParts.BrokenEye />;
      case Emotion.EATING: return <EyeParts.EatingEye />;
      case Emotion.SMART: return <EyeParts.SmartEye />;
      case Emotion.ROCKET: return <EyeParts.RocketEye />;
      case Emotion.ZEN: return <EyeParts.ZenEye />;
      case Emotion.GLITCH: return <EyeParts.GlitchEye />;
  ...
  const eyeColor = useMemo(() => {
    ...
    if (currentEmotion === Emotion.CAT) return '#fcd34d'; // Yellow cat eyes
    if (currentEmotion === Emotion.DOG) return '#a68a64'; // Brownish
    if (currentEmotion === Emotion.ELEPHANT) return '#94a3b8'; // Grey
    if (currentEmotion === Emotion.ANT) return '#ef4444'; // Red aggressive ants
    if (currentEmotion === Emotion.MONKEY) return '#d97706'; // Orange/Brown
    if (currentEmotion === Emotion.BIRD) return '#38bdf8'; // Sky blue
    // New 10 Colors
    if (currentEmotion === Emotion.ALIEN) return '#10b981';
    if (currentEmotion === Emotion.GHOST) return '#e5e7eb';
    if (currentEmotion === Emotion.FIRE) return '#ef4444';
    if (currentEmotion === Emotion.MUSIC) return '#8b5cf6';
    if (currentEmotion === Emotion.BROKEN) return '#dc2626';
    if (currentEmotion === Emotion.EATING) return '#fbbf24';
    if (currentEmotion === Emotion.SMART) return '#06b6d4';
    if (currentEmotion === Emotion.ROCKET) return '#60a5fa';
    if (currentEmotion === Emotion.ZEN) return '#86efac';
    if (currentEmotion === Emotion.GLITCH) return '#bef264';
